<template>    
    <v-card class="pa-5 d-flex justify-center align-center" elevation="11">
        ...
        <span :class="[item.completed ? 'completed' : '', 'itemText']">{{ this.category[item.category_id] }}</span>
...
export default {
    ...
    data: function () {
        return {
            dialog: false,
            category: ['Low-Priority', 'Important', 'High-Priority', 'Needs Attention'],
            editForm: {
                name: '',
                description: '',
                category_id: '',
    ...
    methods: {
        ...
        updateTask() {
            ...
            axios.put('api/v1/user/item-admin/form/' + this.item.id, {
        ...
        updateCheck() {
            axios.put('api/v1/user/item-admin/' + this.item.id, {
                item: this.item
            })
            .then( response => {
                if( response.status == 200){
        ...
        removeItem() {
            axios.delete('api/v1/user/item-admin/' + this.item.id)
            .then( response => {
                if( response.status == 200){
                    this.$emit('itemchanged');
                }
